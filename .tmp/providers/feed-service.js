    ...
    FeedService.prototype.getArticlesForUrl = function () {
        ...
        return this.http.get(url)
            ...
            for (var i = 0; i < objects.length; i++) {
                //console.log('-'+i+'-');
                var item = objects[i];
                var shortDescription = item.description.slice(0, item.description.indexOf("mehr..."));
                if (i == 0) {
                    //console.log('OBJ-0');
                    var newFeedItem = new FeedItem(shortDescription, item.category, item.title, item.thumbnail.url);
